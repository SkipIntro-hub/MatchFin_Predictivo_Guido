...
function calculateProbabilisticModel() {
  ...
  for (let run = 0; run < runs; run += 1) {
    ...
    const stress = normalSample(state.fundingStress, state.stressVolatility, seed + 23);
...
function render() {
  ...
  els.biasText.textContent =
    state.analystBias > 0
      ? "Sesgo optimista: mejora expectativa, pero las senales negativas siguen limitando el score."
      : state.analystBias < 0
        ? "Sesgo conservador: penaliza expectativa para evitar sobreestimacion del escenario."
  ...
  els.fundingStressValue.textContent = `${state.fundingStress}`;
...
function syncControls() {
  ...
  els.fundingStress.value = state.fundingStress;
...
function init() {
  ...
  els.fundingStress.addEventListener("input", (event) => {
    state.fundingStress = Number(event.target.value);
    render();
  });
