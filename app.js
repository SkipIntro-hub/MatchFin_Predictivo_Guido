...
const driverCategories = [
  {
    ...
    evaluate: (ctx) => {
      ...
      return signal(positives, negatives, 62 + ctx.incomeGrowth * 0.9 - Math.max(0, ctx.marketStress - 25) * 0.6);
    },
  },
  {
    id: "eecc",
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      return signal(positives, negatives, 72 - ctx.marketStress * 0.9 + clamp(ctx.ratios.interestCoverage - 2, -2, 6) * 4);
    },
  },
  {
    id: "microindustria",
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      if (ctx.ratios.leverage > 5 || ctx.ratios.interestCoverage < 1.8) negatives.push("Perfil compatible con watch negativo");
      return signal(positives, negatives, 64 + clamp(3 - ctx.ratios.leverage, -4, 3) * 5 + clamp(ctx.ratios.interestCoverage - 2, -2, 5) * 3);
    },
  },
  {
    id: "nosis",
    ...
    evaluate: (ctx) => {
      ...
      if (!pressure && ctx.marketStress < 30) positives.push("Sin senales duras de antecedentes negativos");
      ...
      return signal(positives, negatives, 70 - (pressure ? 18 : 0) - Math.max(0, ctx.debtGrowth - 12) * 0.55 - Math.max(0, ctx.marketStress - 30) * 0.4);
    },
  },
];
...
function drawTemporalRadar(temporal) {
  ...
  const polygon = (values, color, dashed = false, alpha = 1) => {
    ctx.beginPath();
  ...
  polygon(temporal.benchmark, "#13211f", true, 0.35);
  polygon(temporal.previous, "#2f6b9f", false, 0.35);
  polygon(temporal.projected, "#1f8a5b", true, 0.8);
  polygon(temporal.current, "#2f6b9f", false, 1);
...
function drawTemporalTimeline(temporal) {
  ...
  temporal.labels.forEach((label, index) => {
    ...
    ctx.fillText(label, pad.left + index * 122, 12);
  });
}
      ...
      return `
        <div class="dimension-card">
          ...
          <div class="dimension-bar">
            ...
            <i class="current" style="width:${temporal.current[index]}%; background:${colorForDelta(dimDelta)}"></i>
            <i class="projected" style="width:${temporal.projected[index]}%"></i>
            <i class="benchmark" style="left:${temporal.benchmark[index]}%"></i>
          </div>
          <div class="dimension-values">
            <span>Actual ${temporal.current[index]}</span>
            <span>Proy. ${temporal.projected[index]}</span>
            <span>Bench. ${temporal.benchmark[index]}</span>
          </div>
        </div>
      `;
    })
